...
(function($) {
    $.reject = function(opts) {
        var opts = $.extend(true,{
            ...
            display: [], // What browsers to display and their order 
        ...
        if ($.isFunction(opts.beforeReject)) opts.beforeReject(opts); // Customized Function
        ...
        if (browserCheck(opts.reject)) {
            // If user can close and set to remmember close,
            // Initiate cookie function
            ...
            // Load background overlay + Main wrapper +
            // Inner Wrapper w/ opts.header +
            ...
            var _closeReject = function() { // When clicked, fadeOut and remove all elements
                ...
                $('#jr_overlay,#jr_wrap').fadeOut(opts.fadeOutTime,function() {
                    $(this).remove();
                    if ($.isFunction(opts.afterClose)) opts.afterClose(opts); // Customized Function
                });
            ...
            element.find('#jr_overlay').css({ // Creates 'background' (div)
                ...
                padding: 0,
                textAlign: 'center'
            }).children('div.jr_icon').css({ // Icons (div)
                width: 100,
                height: 100,
                margin: '1px auto',
                padding: 0,
                ...
                padding: 0,
                margin: 0
            }).hover(function() { // Underline effect
                $(this).css('textDecoration','underline');
            },function() {
                $(this).css('textDecoration','none');
            }).click(function() { // Make links open in new window
            ...
            if (opts.close && opts.closeESC) {
                $(document).keydown(function(event) { if (event.keyCode == 27) _closeReject(); });
            }
            
            if ($.isFunction(opts.afterReject)) opts.afterReject(opts); // Customized Function
            
            return true;
        }
        else {
            if ($.isFunction(opts.onFail)) opts.onFail(opts); // Customized Function
            return false;
        }
    };
